...
// ------------------ query: show ALL DOCUMENTS rows ------------------
$rows = [];
$documents_BASE_URL = '../uploads/';     // change if your PDFs live elsewhere

try {
  if ($conn instanceof PDO) {
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->query("
      SELECT documents_id, title, description, created_at, file_path
      FROM documents
          ...
          <tr>
            ...
            <td>
              <div class="upload-actions">
               <?php
                $VIEWER = '/franchisee/franchisee_training_docs/view.php';
                ?>
                <a href="<?= $VIEWER ?>?type=documents&id=<?= $row['documents_id'] ?>"
                class="action-btn view-btn btn-view js-view-doc">
