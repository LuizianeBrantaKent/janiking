...
$admin_type  = $_SESSION['user_type'] ?? 'User';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['action'] === 'announcement') {
        ...
        $stmt->execute([$title, $content, $admin_id, $audience]);
    ...
    if ($_POST['action'] === 'message') {
        ...
        $stmt->execute([$admin_type, $admin_id, $subject, $body, $parent_id]);
        ...
        if ($parent_id) {
            ...
            } elseif ($recipient === "All Admin") {
                $conn->prepare("INSERT INTO message_recipients (message_id, receiver_type) VALUES (?, 'AllAdmin')")
                     ->execute([$message_id]);
...
$sql = "
    SELECT m.*, 
           ...
           GROUP_CONCAT(
             CASE 
               ...
               WHEN r.receiver_type='AllAdmin' THEN 'All Admin'
    ...
    WHERE m.parent_message_id IS NULL
      AND (
        ...
        (m.sender_type=:admin_type AND m.sender_ref_id=:admin_id)
        ...
        (
          ...
          (r.receiver_type='Others' AND r.email_override=:admin_email)
          
          OR
          
          -- Sent to my specific user ID
          (r.receiver_type='User' AND r.receiver_ref_id=:admin_id)
          
          OR
          
          -- Sent to my specific franchisee ID (if applicable)
          (r.receiver_type='Franchisee' AND r.receiver_ref_id=:admin_id)
        )
      )
...
$stmt->execute(['admin_id'=>$admin_id, 'admin_type'=>$admin_type, 'admin_email'=>$admin_email]);
    ...
    <div class="tab-pane fade <?=($activeTab==='announcements'?'show active':'')?>" id="announcements">
      <form method="post" class="mt-3">
        ...
        <select name="audience" class="form-control mt-2">
          <option>All</option><option>Franchisee</option><option>Staff</option><option>Admin</option>
        </select>
    ...
    <div class="tab-pane fade <?=($activeTab==='messages'?'show active':'')?>" id="messages">
      ...
      <div class="accordion" id="inboxAccordion">
        <?php foreach($threads as $i=>$t): ?>
          <?php $isSent = ($t['sender_type']===$admin_type && (int)$t['sender_ref_id']===$admin_id); ?>
          <div class="card">
              ...
              <div class="card-body">
                ...
                <?php
                  ...
                  foreach($replies->fetchAll(PDO::FETCH_ASSOC) as $r):
                    $rSent=($r['sender_type']===$admin_type && (int)$r['sender_ref_id']===$admin_id); ?>
...
$('#recipientOther').on('input', function() {
  ...
  if (searchTerm.length > 2) {
    $.get('admin_announcement/search_contacts.php', { q: searchTerm }, function(data) {
